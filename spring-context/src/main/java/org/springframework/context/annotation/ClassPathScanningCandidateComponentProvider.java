...
public class ClassPathScanningCandidateComponentProvider implements EnvironmentCapable, ResourceLoaderAware {
	...
	public static MultiValueMap<String, String> INDEX = new LinkedMultiValueMap<>();
					...
					if (isCandidateComponent(metadataReader)) {
						...
						if (isCandidateComponent(sbd)) {
							...
							for (TypeFilter filter : this.includeFilters) {
								if (filter.match(metadataReader, getMetadataReaderFactory())) {
									String stereotype = extractStereotype(filter);
									INDEX.add(metadataReader.getClassMetadata().getClassName(), stereotype);
								}
							}
						...
						for (String typeAnnotation : metadataReader.getAnnotationMetadata().getAnnotationTypes()) {
							if (typeAnnotation.startsWith("jakarta.") || typeAnnotation.startsWith("javax.")) {
								INDEX.add(metadataReader.getClassMetadata().getClassName(), typeAnnotation);
							}
						}
						if (traceEnabled) {

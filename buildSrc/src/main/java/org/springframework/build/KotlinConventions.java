...
public class KotlinConventions {
	...
	void apply(Project project) {
		if (project.equals(project.getRootProject())) {
			project.getExtensions().configure(DokkaExtension.class, dokka -> {
				dokka.getModuleName().set("spring-framework");
				dokka.getDokkaPublications().named("html").configure(publication -> {
					publication.getIncludes().from(project.getLayout().getProjectDirectory()
							.file("framework-docs/src/docs/api/dokka-overview.md"));
				});
			});
		}
	...
	private void configure(Project project, DokkaExtension dokka) {
		System.out.println("Configure DokkaTask for spring module" + project.getName());
		dokka.getDokkaSourceSets().forEach(sourceSet -> {
			...
			sourceSet.sourceLink(sourceLink -> {
				sourceLink.remoteUrl("https://docs.spring.io/spring-framework/docs/current/javadoc-api/");
			});

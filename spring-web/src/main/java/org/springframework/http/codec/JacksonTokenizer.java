...
final class JacksonTokenizer {
	...
	public static Flux<TokenBuffer> tokenize(Flux<DataBuffer> dataBuffers,
		...
		try {
			JsonParser parser;
			try {
				// TODO https://github.com/FasterXML/jackson-databind/issues/5110
				DeserializationContextExt context = objectMapper._deserializationContext();
				parser = context.assignAndReturnParser(objectMapper.tokenStreamFactory().createNonBlockingByteBufferParser(context));
			}

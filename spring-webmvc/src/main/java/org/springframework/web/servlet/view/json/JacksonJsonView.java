...
public class JacksonJsonView extends AbstractJacksonView {
	...
	/**
	 * Construct a new {@code JacksonJsonView} loading {@link tools.jackson.databind.JacksonModule}s
	 * using JDK {@link java.util.ServiceLoader} facility and setting the content type
	...
	public JacksonJsonView() {
		...
		super(JsonMapper.builder()
				...
				.addModules(MapperBuilder.findModules(JacksonJsonHttpMessageConverter.class.getClassLoader()))
				.build(), DEFAULT_CONTENT_TYPE);
	}

	/**
	 * Construct a new {@code JacksonJsonView} using the provided

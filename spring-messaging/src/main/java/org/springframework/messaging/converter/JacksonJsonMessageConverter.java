...
public class JacksonJsonMessageConverter extends AbstractMessageConverter {
	...
	/**
	 * Construct a {@code JacksonJsonMessageConverter} with a default {@link ObjectMapper},
	...
	public JacksonJsonMessageConverter(MimeType... supportedMimeTypes) {
		...
		this.objectMapper = JsonMapper.builder().disable(MapperFeature.DEFAULT_VIEW_INCLUSION).build();
	}

	/**
	 * Construct a {@code JacksonJsonMessageConverter} with a custom {@link ObjectMapper},
	 * supporting the {@code application/json} MIME type with {@code UTF-8} character set.

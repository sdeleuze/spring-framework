...
public interface BeanRegistry {
	...
	/**
	 * Register a bean from the given bean class, customizing it with the spec
	 * callback. The bean will be instantiated using the supplier that can be
	 * configured in the spec callback, or will be tentatively instantiated with its
	 * {@link BeanUtils#getResolvableConstructor resolvable constructor} otherwise.
	 * @param beanClass the class of the bean
	 * @param spec callback to customize other bean properties than the
	 * name for example.
	 * @return the generated bean name
	 */
	<T> String registerBean(Class<T> beanClass, Consumer<Spec<T>> spec);
	 ...
	 * @param name the name of the bean
	 * @param beanClass the class of the bean
	 * @param spec callback to customize other bean properties than the name
	 */
	<T> void registerBean(String name, Class<T> beanClass, Consumer<Spec<T>> spec);
	...
	interface Spec<T> {
		...
		/**
		 * The sort order for the targeted bean. This is analogous to the
